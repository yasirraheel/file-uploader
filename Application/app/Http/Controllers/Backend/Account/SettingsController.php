...
class SettingsController extends Controller
{
    ...
    public function detailsUpdate(Request $request)
    {
        ...
        if ($request->has('avatar')) {
            if ($this->admin()->avatar == 'images/avatars/default.png') {
                ...
                $uploadAvatar = vImageUpload($request->file('avatar'), 'images/avatars/admins/', '110x110', null, $this->admin()->avatar);
            }
        } else {
    ...
    public function securityUpdate(Request $request)
    {
        ...
        if (!(Hash::check($request->get('current-password'), $this->admin()->password))) {
            toastr()->error(__('Your current password does not matches with the password you provided.'));
            return back();
        }
        if (strcmp($request->get('current-password'), $request->get('new-password')) == 0) {
            toastr()->error(__('New Password cannot be same as your current password. Please choose a different password.'));
            return back();
        }
        $update = Admin::where('id', $this->admin()->id)->update([
